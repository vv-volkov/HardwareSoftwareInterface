...
void* mm_malloc (size_t size) {
  ...
  if( extraSize >= MIN_BLOCK_SIZE ){
    ...
    ptrNextBlock->sizeAndTags = extraSize | TAG_PRECEDING_USED;
    *((size_t*)POINTER_ADD(ptrNextBlock, SIZE(ptrNextBlock->sizeAndTags) - WORD_SIZE)) = extraSize | TAG_PRECEDING_USED;
...
void mm_free (void *ptr) {
  ...
  *((size_t *)POINTER_ADD(blockInfo,payloadSize-WORD_SIZE)) &= ~TAG_USED;
  blockInfo->next = NULL;
  blockInfo->prev = NULL;
  followingBlock = POINTER_ADD(blockInfo, payloadSize);
  *((size_t *)POINTER_ADD(followingBlock,SIZE(followingBlock->sizeAndTags)-WORD_SIZE)) &= ~TAG_PRECEDING_USED;
